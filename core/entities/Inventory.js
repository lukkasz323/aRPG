    ...
    constructor(canvas) {
        ...
        for (let y = 0; y < this.backpackSize.y; y++) {
            for (let x = 0; x < this.backpackSize.x; x++) {
                this.children.push(new ItemSlot(new Vector2(backpackOrigin.x + (x * backpackCellSize), backpackOrigin.y + (y * backpackCellSize)), new Vector2(backpackCellSize, backpackCellSize), undefined, undefined, undefined, alpha, renderScreenSpace, false, true, 21));
            }
        }
        this.close();
    }
